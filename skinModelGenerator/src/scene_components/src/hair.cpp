...
{
    float hairy_factor = simulation.arm.hairDensityFactor * .02;
    ...
    for(int i = 0; i < (int) sampler->points.size(); i++){
        ...
        z = (sampler->points[i].y) * (simulation.arm.z / 2); //Gets a number between -simulation.arm.z/2 and + simulation.arm.z/2
        theta = sampler->points[i].x * PI;
        points[0][0] = (simulation.arm.x - 1.525) * cos(theta);
        points[0][1] = (simulation.arm.y - 1.525) * sin(theta);
        points[0][2] = z;
        ...
        z += 6;
        points[1][0] = (simulation.arm.x + 3.05) * cos(theta);
        points[1][1] = (simulation.arm.y + 3.05) * sin(theta);
        points[1][2] = z;

        z += 6;
        theta_prime = theta + (.01 * PI);
        points[2][0] = (simulation.arm.x + 3.05) * cos(theta_prime);
        points[2][1] = (simulation.arm.y + 3.05) * sin(theta_prime);
        points[2][2] = z;

        z += 6;
        points[3][0] = (simulation.arm.x + 3.05) * cos(theta);
        points[3][1] = (simulation.arm.y + 3.05) * sin(theta);
        points[3][2] = z;
